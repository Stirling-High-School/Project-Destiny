...
export default function ActualForm({ profile, id }) {
    ...
    const { isSubmitting, isSubmitted, isSubmitError, submitErrorComponent } =
        submitted;
    ...
    const submitForm = (e) => {
        ...
        function postData() {
            ...
            for (let i in submitValues.data.choices) {
                ...
                submitValues.data.choices[i].weight =
                    submitValues.data.choices[i].weight.label;
            }
    ...
    if (isSubmitting) {
        ...
        return (
            <MessageComponent
                ...
                description={
                    "You should recieve an email confirmation shortly."
                }
            />
        );
