...
2. To set up the website, first you need to download and install [Node.js](https://nodejs.org/en/download/). Once Node.js is installed, clone the `course-choice-frontend` folder from the GitHub repository to your computer. Now open up a command line in that folder (this can be done by right clicking inside the folder and selecting `Open in terminal` on Windows/Linux). Now run `npm install`. This will install all of the dependencies required to build the website. Once this is completed, run `npm build`. This will create a folder called `build`.
3. Now we can deploy to Firebase! First, run `npm install -g firebase-tools` to install the Firebase CLI. When it has finished installing, run `firebase login` to log in. Once logged in, we can initialise the project by running `firebase init`:
